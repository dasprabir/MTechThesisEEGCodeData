...
topoplotIndie(sum(logical(pli_mat)),EEG.chanlocs,'plotrad',.53,'maplimits',[0 25]);
title('Connectivity degree, phase-lag index')

subplot(122)
topoplotIndie(sum(logical(ispc_mat)),EEG.chanlocs,'plotrad',.53,'maplimits',[0 25]);
...
for fi=1:length(freqs2plot)
    for ti=1:length(times2plot)
        ...
        topoplotIndie(squeeze(tf_degree(:,dsearchn(frex',freqs2plot(fi)),dsearchn(times2save',times2plot(ti)))),EEG.chanlocs,'plotrad',.53,'maplimits',clim,'electrodes','off');
...
for fi=1:length(freqs2plot)
    for ti=1:length(times2plot)
        ...
        figure(figh1)
        subplot(length(freqs2plot),length(times2plot),ti+(fi-1)*length(times2plot))
        %topoplotIndie(clustcoef,EEG.chanlocs,'plotrad',.53,'maplimits',clim,'electrodes','off');
