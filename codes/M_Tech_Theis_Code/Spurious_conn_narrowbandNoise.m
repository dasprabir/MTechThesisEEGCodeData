...
for ti=1:pnts
    ...
    synchTS(ti) = abs(mean(exp(1i*( angles1(tidx)-angles2(tidx) ))));   %ISPC
end

% Average synchronisation   avgSync = mean(synchTS)
...
%% done.
figure,
plot(linspace(0,srate,pnts),smooth(abs(fft(detrend(synchTS)))))
set(gca,'xlim',[0 10])